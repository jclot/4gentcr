...
export default function PersonalDataScreen({ navigation }: any) {
  ...
  const persistAvatar = async (nextAvatar: string) => {
    ...
    try {
      ...
      confirm({
        title: 'No se pudo actualizar foto',
  ...
  const pickFromGallery = async () => {
    ...
    if (!permission.granted) {
      confirm({
        ...
        message: 'Necesitamos permiso de galeria para elegir tu foto de perfil.',
        confirmLabel: 'OK',
        onConfirm: () => {},
      });
      return;
    ...
    if (!result.canceled && result.assets.length > 0) {
      const normalized = await normalizeAvatarUri(result.assets[0].uri);
      await persistAvatar(normalized);
    }
  };
  ...
  const takePhoto = async () => {
    ...
    if (!permission.granted) {
      confirm({
        title: 'Permiso requerido',
        message: 'Necesitamos permiso de camara para tomar tu foto de perfil.',
        confirmLabel: 'OK',
        onConfirm: () => {},
      });
      return;
        ...
        >
          <View style={styles.avatarSection}>
            ...
            <Text style={styles.avatarHint}>
              {avatarSaving ? 'Guardando foto...' : 'Tu foto se vera en perfil, comunidad y gestion.'}
            </Text>
            <View style={styles.avatarActions}>
              <TouchableOpacity style={styles.avatarActionBtn} onPress={pickFromGallery}>
                <Text style={styles.avatarActionText}>Elegir foto</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.avatarActionBtn} onPress={takePhoto}>
                <Text style={styles.avatarActionText}>Tomar foto</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.avatarActionBtn, styles.avatarDeleteBtn]} onPress={removeAvatar}>
                <Text style={[styles.avatarActionText, { color: Colors.danger }]}>Eliminar</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View style={styles.card}>
            ...
            />
            <Input
              label="Telefono asociado a SINPE"
          ...
          <View style={styles.card}>
            ...
            <View style={styles.infoBox}>
              ...
              <Text style={styles.infoText}>
                Para cambiar correo o cedula debes contactar a soporte.
              </Text>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  avatarHint: { fontSize: 13, color: Colors.textSecondary, textAlign: 'center' },
  ...
  avatarDeleteBtn: { borderColor: Colors.dangerLight },
  avatarActionText: { fontSize: 12, fontWeight: '700', color: Colors.textPrimary },
